...
/*  if (success == 1)
     {
      ...
      sema_up (&thread_current()->chp->sema_wait);
     }
  else
    {
      sema_up (&thread_current()->chp->sema_wait);
...
//   while (!list_empty (&thread_current()->chp->sema_wait.waiters))
//      sema_up (&thread_current()->chp->sema_wait);
   //if(thread_current()->parent != NULL)
      //sema_down (&thread_current()->chp->sema_exit); 
//Close files owned
   close_all_files();
